...
미확인 재시도는 **배치 단위로 `WAKE_UNCONFIRMED_MAX` 회에서 소켓 쓰기 자체를 멈춘다**
(백오프만 늘리던 시절엔 로그가 '재주입 중단'이라 찍히면서 5분마다 같은 봉투를 계속 밀어 넣었다 —
실측 2026-08-22 라이브: 한 세션 10회·다른 세션 5회 중복 주입, 격리 재현 20초에 43회 → 고친 뒤 2회).
멈춘 뒤에도 항목은 캐시에 남아 훅·부활 폴백이 집어가고, 늦은 활동/영수증이 오면 그때 확정한다.
**새 메시지는 상한과 무관하게 자기 몫의 시도를 받는다**(상한은 그 배치에만 걸린다).
